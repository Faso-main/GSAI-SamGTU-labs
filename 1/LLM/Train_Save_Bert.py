...
class Search:
    ...
    def __init__(self, hashmap: Inquiry, confidence: float, track_time: bool, 
        ...
        if train_model and questions is not None and answers is not None:
            self.trainer = BertTrainer(num_epochs=2)  # инициализация тренера(кол-во эпоха)
            self.trainer.train(questions, answers, save_path='model.pth')  # Обучение модели
            self.trainer.load_model('model.pth')  # Загрузка обученной модели
        else:
...
class BertTrainer:
    ...
    def __init__(self, model_name='DeepPavlov/rubert-base-cased', num_epochs=1, learning_rate=1e-5):
        ...
        self.device = torch.device("cpu")  # процессор: CPU, видеопамять: "cuda" для GPU
...
Developer_mode = True  # (True - показ времени и точности в ответе, False - вывод только ответа)

# Поиск ответов, на основе входных данных
response = Search(
    inquiry,
    confidence=0.76,track_time=Developer_mode,
    show_accuracy=Developer_mode,train_model=Train_new_model,
    questions=questions if Train_new_model else None,answers=answers if Train_new_model else None
)
